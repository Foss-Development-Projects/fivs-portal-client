      ...
      {zoomedImage && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-2xl z-[250] flex items-center justify-center p-4">
          ...
          <button
            ...
            className="fixed top-6 right-6 md:top-10 md:right-10 z-[300] bg-white text-black w-12 h-12 md:w-16 md:h-16 rounded-full flex items-center justify-center shadow-2xl hover:bg-red-500 hover:text-white transition-all transform active:scale-95"
            title="Close Preview"
          >
          ...
          <div className="max-w-6xl w-full h-full flex flex-col items-center justify-center relative animate-scaleIn">
            {(() => {
              ...
              if (isImage) {
                return (
                  <img
                    src={zoomedImage}
                    className="max-w-full max-h-[85vh] object-contain shadow-2xl rounded-2xl border-4 border-white/10"
                    alt="Zoomed View"
                  />
                );
              } else if (isPdf) {
            ...
            <p className="mt-6 text-white/40 text-[10px] uppercase font-black tracking-[0.3em] hidden md:block">Click outside or use ESC to exit</p>
          </div>
        </div>
      )}
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-gray-800 p-8 rounded-[3rem] shadow-sm border border-gray-100 dark:border-gray-700 grid grid-cols-1 lg:grid-cols-4 gap-4">
            <div className="relative lg:col-span-2">
              ...
              <input type="text" placeholder="Quick Search ID, Name, Vehicle..." className="w-full pl-12 pr-4 py-4 rounded-2xl border-2 border-gray-50 dark:bg-gray-700 dark:border-gray-600 dark:text-white outline-none focus:border-[#2E7D32]" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
            </div>
            ...
            <select className="px-4 py-4 rounded-2xl border-2 border-gray-50 dark:bg-gray-700 dark:text-white font-bold text-xs" value={filterAggregator} onChange={e => setFilterAggregator(e.target.value)}>
            ...
            <div className="flex items-center space-x-6">
              <div className="hidden md:flex space-x-2">
                {Array.from({ length: totalSteps }, (_, i) => i + 1).map(step => (
                  <div key={step} className={`w-10 h-1.5 rounded-full transition-all ${currentStep === step ? 'bg-[#2E7D32]' : currentStep > step ? 'bg-green-200' : 'bg-gray-200 dark:bg-gray-700'}`} />
                ))}
              </div>
                ...
                <div className="space-y-10 animate-fadeIn">
                  ...
                  <section className="p-8 bg-blue-50 dark:bg-blue-900/20 rounded-[2.5rem] border border-blue-100 dark:border-blue-800 space-y-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      <SmartInput label="Owner's Name" value={formData.ownerName} onChange={v => setFormData(prev => ({ ...prev, ownerName: v }))} />
                      <SmartInput label="Vehicle Reg No" value={formData.vehicleRegNo} onChange={v => setFormData(prev => ({ ...prev, vehicleRegNo: v }))} />
                      <SmartInput label="Policy No" value={formData.policyNo} onChange={v => setFormData(prev => ({ ...prev, policyNo: v }))} />
                      <SmartInput label="Engine No" value={formData.engineNo} onChange={v => setFormData(prev => ({ ...prev, engineNo: v }))} />
                      <SmartInput label="Chassis No" value={formData.chassisNo} onChange={v => setFormData(prev => ({ ...prev, chassisNo: v }))} />
                      <SmartInput label="Policy Start Date" value={formData.policyStartDate} onChange={v => setFormData(prev => ({ ...prev, policyStartDate: v }))} type="date" />
                      <SmartInput label="Policy End Date" value={formData.policyEndDate} onChange={v => setFormData(prev => ({ ...prev, policyEndDate: v }))} type="date" />
                      <SmartInput label="Renewal Date" value={formData.renewalDate} onChange={v => setFormData(prev => ({ ...prev, renewalDate: v }))} type="date" className="bg-orange-50 border-orange-100" />
                    </div>
                    <SmartInput label="Owner Address" value={formData.ownerAddress} onChange={v => setFormData(prev => ({ ...prev, ownerAddress: v }))} />
                ...
                <div className="space-y-10 animate-fadeIn">
                  ...
                  <section className="p-8 bg-blue-50 dark:bg-blue-900/20 rounded-[2.5rem] border border-blue-100 dark:border-blue-800 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <SmartInput label="Vehicle Class" value={formData.vehicleClass} onChange={v => setFormData(prev => ({ ...prev, vehicleClass: v }))} />
                    </div>
                    <div className="flex justify-between items-center pt-4">
                      <button onClick={prevStep} className="text-gray-400 font-black uppercase text-[10px] tracking-widest">Back</button>
                      <button onClick={nextStep} className="bg-[#2E7D32] text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl flex items-center">
              ...
              {currentStep === 4 && (
                <div className="space-y-10 animate-fadeIn">
                  ...
                  <section className="p-8 bg-blue-50 dark:bg-blue-900/20 rounded-[2.5rem] border border-blue-100 dark:border-blue-800 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <SmartInput label="PAN Number" value={formData.panNo} onChange={v => setFormData(prev => ({ ...prev, panNo: v }))} />
                      <SmartInput label="Date of Birth" value={formData.dob} onChange={v => setFormData(prev => ({ ...prev, dob: v }))} type="date" />
                    </div>
                    <div className="flex justify-between items-center pt-4">
                      <button onClick={prevStep} className="text-gray-400 font-black uppercase text-[10px] tracking-widest">Back</button>
                      <button onClick={nextStep} className="bg-[#2E7D32] text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl flex items-center">
              ...
              {currentStep === 5 && (
                <div className="space-y-10 animate-fadeIn">
                  ...
                  <section className="p-8 bg-blue-50 dark:bg-blue-900/20 rounded-[2.5rem] border border-blue-100 dark:border-blue-800 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <SmartInput label="Aadhaar Number" value={formData.aadhaarNo} onChange={v => setFormData(prev => ({ ...prev, aadhaarNo: v }))} />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 animate-fadeIn">
                    ...
                    <section className="space-y-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="p-4 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm transition-all focus-within:border-[#2E7D32]">
                          ...
                          <select className="w-full bg-transparent outline-none font-bold text-xs dark:text-white" value={formData.webAggregator} onChange={e => setFormData(prev => ({ ...prev, webAggregator: e.target.value }))}>
                            {WEB_AGGREGATORS.map(a => <option key={a} value={a}>{a}</option>)}
                          </select>
                        </div>
                        <div className="p-4 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm transition-all focus-within:border-[#2E7D32]">
                          <label className="block text-[8px] font-black uppercase text-gray-400 mb-1">Insurance Company</label>
                          <select className="w-full bg-transparent outline-none font-bold text-xs dark:text-white" value={formData.insuranceCompany} onChange={e => setFormData(prev => ({ ...prev, insuranceCompany: e.target.value }))}>
                            {INSURANCE_COMPANIES.map(c => <option key={c} value={c}>{c}</option>)}
                          </select>
                        </div>
                        <div className="p-4 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm transition-all focus-within:border-[#2E7D32]">
                          <label className="block text-[8px] font-black uppercase text-gray-400 mb-1">Policy Type</label>
                          <select className="w-full bg-transparent outline-none font-bold text-xs dark:text-white" value={formData.insuranceType} onChange={e => setFormData(prev => ({ ...prev, insuranceType: e.target.value as any }))}>
                      ...
                      <div className="p-6 bg-gray-50 dark:bg-gray-900/50 rounded-2xl border border-gray-100 dark:border-gray-700 space-y-4">
                        ...
                        {formData.isNameTransfer && (
                          <div className="animate-fadeIn">
                            <SmartInput label="Estimated Name Transfer Date" value={formData.nameTransferDate} onChange={v => setFormData(prev => ({ ...prev, nameTransferDate: v }))} type="date" />
                          </div>
                        )}
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <SmartInput label="Owner Phone No" value={formData.ownerPhone} onChange={v => setFormData(prev => ({ ...prev, ownerPhone: v }))} />
                        <SmartInput label="Owner Profession" value={formData.ownerProfession} onChange={v => setFormData(prev => ({ ...prev, ownerProfession: v }))} />
                      </div>
                  ...
                  <div className="space-y-8">
                    <section className="p-8 bg-gray-50 dark:bg-gray-900 rounded-[2.5rem] border border-gray-100 dark:border-gray-700 space-y-6">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <SmartInput label="Broker Name" value={formData.brokerName} onChange={v => setFormData(prev => ({ ...prev, brokerName: v }))} />
                        <SmartInput label="Broker Phone" value={formData.brokerPhone} onChange={v => setFormData(prev => ({ ...prev, brokerPhone: v }))} />
                        <SmartInput label="Broker Profession" value={formData.brokerProfession} onChange={v => setFormData(prev => ({ ...prev, brokerProfession: v }))} />
                        <SmartInput label="Broker Address" value={formData.brokerAddress} onChange={v => setFormData(prev => ({ ...prev, brokerAddress: v }))} />
                      </div>
                    </section>
                    ...
                    <section className="p-8 bg-gray-50 dark:bg-gray-900 rounded-[2.5rem] border border-gray-100 dark:border-gray-700 space-y-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <SmartTextArea label="Special Notes" value={formData.specialNotes} onChange={v => setFormData(prev => ({ ...prev, specialNotes: v }))} />
                        <SmartTextArea label="Remarks" value={formData.remarks} onChange={v => setFormData(prev => ({ ...prev, remarks: v }))} />
                      </div>
                    </section>
...
const SmartInput = ({ label, value, onChange, type = "text", required = false, className = "" }: any) => (
  <div className={`p-4 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm transition-all focus-within:border-[#2E7D32] focus-within:ring-4 focus-within:ring-green-500/5 ${className}`}>
    ...
    <input type={type} className="w-full bg-transparent outline-none font-bold text-xs dark:text-white" value={value || ''} onChange={e => onChange(e.target.value)} />
  </div>
);

const SmartTextArea = ({ label, value, onChange, className = "" }: any) => (
  <div className={`p-4 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm transition-all focus-within:border-[#2E7D32] focus-within:ring-4 focus-within:ring-green-500/5 ${className}`}>
    <label className="block text-[8px] font-black uppercase text-gray-400 mb-1">{label}</label>
    <textarea rows={3} className="w-full bg-transparent outline-none font-bold text-xs dark:text-white resize-none" value={value || ''} onChange={e => onChange(e.target.value)} />
  </div>
);
